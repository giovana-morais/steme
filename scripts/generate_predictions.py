...
def generate_predictions(mirdata_dataset, mirdata_dataset_data_folder,
    ...
    predictions_folder = os.path.join(
        DATA_FOLDER, f"predictions/{model_name}_fixed_shift")
    ...
    with h5py.File(track_file, "r") as hf:
        ...
        for idx, track_id in enumerate(hf.keys()):
            ...
            for i in range(T.shape[1]):
                s1, sh1, _, _, _ = dt.get_tempogram_slices(
...
def main(
    ...
    gtzan_data_folder = os.path.join(
        PRE_COMPUTED_DATA_FOLDER,
        f"gtzan_{tmin}_{n_bins}_{bins_per_octave}_fourier")
