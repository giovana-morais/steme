...
class Track:
    ...
    def __repr__(self):
        ...
        attributes = [v for v in dir(self) if not v.startswith(
            "_") and v not in properties]
...
class Dataset:
    def __init__(
            self,
            data_home='./datasets/brid/audio',
            annotations_home='./datasets/brid/',
            dataset_name='brid',
        ...
        for root, dirs, files in os.walk(self.data_home):
            for name in files:
                if not name == '.DS_Store':
                    aux_dict = {
                        'audio': os.path.join(
                            root, name), 'beats': os.path.join(
                            beats_home, name.replace(
                                '.wav', '.beats')), 'tempo': os.path.join(
                            tempo_home, name.replace(
                                '.wav', '.bpm'))}
